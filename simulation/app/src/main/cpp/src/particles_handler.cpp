...
ParticlesHandler::ParticlesHandler(Physics &physics, int num)
    : physics(physics), num(num), pool(std::thread::hardware_concurrency()),
      thread_count(std::thread::hardware_concurrency()) {
  isInitialized = false;
}
...
void ParticlesHandler::initParticles(InitType type) {
  ...
  case InitType::two_lines:
    for (int i = 0; i < num; i++) {
      ...
      glm::vec3 initialVel(0.0f, 0.0f, 0.0f);
      float xPos =
          FIELD_WIDTH * (i % 2 ? (i / (float)num) - 1 : 1 - (i / (float)num));
      float yPos = FIELD_HEIGHT * (2 * (i / (float)num) - 1);
      float zPos = FIELD_DEPTH * (2 * (i / (float)num) - 1);
      glm::vec3 initialPos(xPos, yPos, zPos);
      particles.push_back(Particle(initialPos, initialVel));
...
inline void ParticlesHandler::bindPosition(Particle &particle) {
  particle.position.x =
      std::clamp(particle.position.x, -FIELD_WIDTH, FIELD_WIDTH);
  particle.position.y =
      std::clamp(particle.position.y, -FIELD_HEIGHT, FIELD_HEIGHT);
  particle.position.z =
      std::clamp(particle.position.z, -FIELD_DEPTH, FIELD_DEPTH);
}
...
void ParticlesHandler::loadPositionsFromFile(const std::string &filePath) {
  if (isInitialized) {
    LOGE("particles_handler",
         "Particles have already been initialized, ignoring file load.");
  ...
  std::vector<float> lats(numParticles), lons(numParticles),
      depths(numParticles);
  ...
  for (size_t i = 0; i < numParticles; i++) {
    particlesPos[3 * i] =
        FIELD_WIDTH * ((lons[i] / maxLon) * 2 - 1); // X (longitude)
    particlesPos[3 * i + 1] =
        FIELD_HEIGHT * ((lats[i] / maxLat) * 2 - 1); // Y (latitude)
    particlesPos[3 * i + 2] =
        FIELD_DEPTH * ((depths[i] / maxDepth) * 2 - 1); // Z (depth)
  }
